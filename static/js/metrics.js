...
{ // Yandex reklama
  ...
  window.addYaRTB_Block = function(blid,p_dark,rtbid,typ){ 
    function ads(blid,p_dark,rtbid,typ){ if(document.getElementById('yandex_rtb_'+blid)){ 
      (new IntersectionObserver(function(es){ 
        es.forEach((e)=>{ if(e.isIntersecting){ switch(typ){
          case 'widget': window.yaContextCb.push(()=>{Ya.Context.AdvManager.renderWidget({darkTheme:p_dark, blockId:'C-A-7295044-'+rtbid, renderTo:'yandex_rtb_'+blid});});
    ...
    YA_TMR.push( setInterval((blid,p_dark,rtbid,typ)=>{ ads(blid,p_dark,rtbid,typ) }, (Math.random()*20+9)*1000) );
    setTimeout((blid,p_dark,rtbid,typ)=>{ ads(blid,p_dark,rtbid,typ) });
  };
