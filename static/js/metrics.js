...
{ // Yandex reklama
  ...
  window.YA_RTB = {1:3, 2:4, 3:9, 4:6, 5:7, 6:8, 7:5, feed:10};
  window.YA_TMR = [];
  window.addYaRTB_Block = function(blid,p_dark,rtbid,typ){ 
    function ads(){ if(document.getElementById('yandex_rtb_'+blid)){ 
      (new IntersectionObserver(function(es){ 
        es.forEach((e)=>{
          if(e.isIntersecting){ setTimeout(()=>{ window.yaContextCb.push(function(){Ya.Context.AdvManager.render({darkTheme:p_dark, blockId:'R-A-7295044-'+rtbid, renderTo:'yandex_rtb_'+blid, type:typ});});}, 0); }
        });
