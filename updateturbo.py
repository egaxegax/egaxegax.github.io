...
def main(path='.', icount=0, surl='egax.ru', sdir=''):
  ...
  try:
    ...
    chabout = open(os.path.join(path, 'about.md'), encoding='utf-8', newline='\n').read()
  except:
    raise
...
<turbo:content><![CDATA[<header><h1>{mroot}{subj}</h1><h2>{titl}</h2></header>{text}]]></turbo:content>
<pubDate>{pdate}</pubDate>
</item>""".format(mroot=mroot, subj=subj, titl=titl, troot=troot, trsubj=tr_cut(subj), trtitl=tr_cut(titl), link=surl +'/'+ sind +'.html?'+ tr(subj) +'/'+ tr(titl) +ulink, text=re.sub('\n\s*',' ',text), pdate=pdate))
  
    rsstext = """<?xml version="1.0" encoding="UTF-8"?>
<rss xmlns:yandex="http://news.yandex.ru" xmlns:media="http://search.yahoo.com/mrss/" xmlns:turbo="http://turbo.yandex.ru" version="2.0">
<channel>
<title>{chtitl}</title>
<link>https://{surl}/{sind}.html</link>
<description>{chabout}</description>{items}
</channel>
</rss>
""".format(surl=surl, sind=sind, chtitl=chtitl, chabout=chabout, items=''.join(items))

    io.open(os.path.join(sdir or path, 'rss-trb'+surls.get(surl, '')+ rcount +'.xml'), 'w', encoding='utf-8', newline='\n').write(rsstext)
    print(sdir+'/rss-trb'+surls.get(surl, '')+ rcount +'.xml', len(items))
