...
with urllib.request.urlopen("https://habr.com/ru/rss/articles/?fl=ru") as purl:
  ...
  if rsstext:
    ...
    for channel in root.findall("channel"):
      ...
      gyear = time.strftime('%y', gdt)
      with open(os.path.join(cdir, str(gdt.tm_year), '{y}{m}{d} 0300.md'.format(y=gyear, m=('%02d' % gdt.tm_mon), d=('%02d' % gdt.tm_mday))), 'w+', encoding='utf-8', newline='\n') as fp:
